...
export default function SignUpPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Save role cookie for long-term logic
      Cookies.set('userRole', user._type, { expires: 30 });

      // Determine destination
      const isLocal = typeof window !== 'undefined' && window.location.hostname === 'localhost';

      const vendorURL = process.env.NEXT_PUBLIC_VENDOR_DASHBOARD_URL || (isLocal ? 'http://localhost:3001/dashboard' : 'https://vendor.fasmotorsports.com/dashboard');
      const customerURL = process.env.NEXT_PUBLIC_CUSTOMER_DASHBOARD_URL || (isLocal ? 'http://localhost:3000/dashboard' : 'https://fasmotorsports.com/dashboard');
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-fit text-white px-4"
      ...
      <form onSubmit={handleRegister}
        ...
        <button type="submit"
                className="btn btn-primary font-ethno tracking-wide px-6 py-3 rounded-md shadow-md w-full">
          Register
        </button>
